...
export async function handleConversationMessage(
  ...
  switch (state) {
    ...
    case "SELECTING_DATE":
      ...
      return;
    case "SELECTING_SLOT":
      await handleSelectingSlot(admin, salon, customerPhone, userText, incoming, ctx);
      return;
...
async function handleSelectingDate(
  ...
  const slotRes = await getAvailableSlots(admin, salon.id, day, totalDur);
  if (!slotRes.ok) {
    await sendAuth(salon, (pid, tok) =>
      sendWhatsAppText(pid, tok, { toE164: customerPhone, body: slotRes.reason })
    );
    return;
  }

  const starts = slotRes.slots.map((s) => s.startUtc.toISOString());
  const lines = slotRes.slots.map((s) => `${s.index}. ${s.label}`).join("\n");

  await ensureConversationRow(admin, salon.id, customerPhone, "SELECTING_SLOT", {
    ...ctx,
    selectedDayIso: day.toISOString(),
    slotStarts: starts,
    lastListMax: slotRes.slots.length,
  });

  if (slotRes.slots.length <= 10) {
    await sendAuth(salon, (pid, tok) =>
      sendWhatsAppList(pid, tok, {
        toE164: customerPhone,
        bodyText: "Pick a time slot:",
        buttonText: "Times",
        sections: [
          {
            title: "Slots",
            rows: slotRes.slots.map((s) => ({
              id: `slot_${s.index}`,
              title: s.label,
...
async function handleSelectingSlot(
  ...
  const max = ctx.lastListMax ?? starts.length;
  let num: number | null = null;

  if (incoming.kind === "interactive" && incoming.id.startsWith("slot_")) {
    num = Number(incoming.id.replace("slot_", ""));
  } else {
    num = Number(body.trim());
  }
  ...
  if (!Number.isFinite(num) || num < 1 || num > max) {
    await sendAuth(salon, (pid, tok) =>
      sendWhatsAppText(pid, tok, {
        toE164: customerPhone,
        body: `Send a number between 1 and ${max}.`,
      })
    );
    return;
  }

  const startIso = starts[num - 1];
  if (!startIso) {
    await sendAuth(salon, (pid, tok) =>
      sendWhatsAppText(pid, tok, {
        toE164: customerPhone,
        body: "That slot is invalid. Try again.",
      })
    );
    return;
  }
  ...
  await ensureConversationRow(admin, salon.id, customerPhone, "CONFIRMING_NAME", {
    ...ctx,
    selectedSlotIndex: num,
  });

  const label = formatSlotLabel(parseISO(startIso));
  await sendAuth(salon, (pid, tok) =>
    sendWhatsAppText(pid, tok, {
      toE164: customerPhone,
      body: `Great — ${label}. What's your name for the booking?`,
    })
  );
}
